...
# HIGH CONFIDENCE OTC SIGNAL BOT WITH MARTINGALE MAX GAIN
...
def detect_trend(price_list):
    ...
    strength = max(70, min(strength,98))  # min 70%, max 98%
    score = min(50 + strength*0.5,100)    # 70-99% range
    direction = None
...
# ================================
# SEND TELEGRAM SIGNAL WITH MAX GAIN
# ================================
def send_signal(pair,direction,score,strength):
    ...
    max_gain = sum([1.0*(i+1) for i in range(MAX_MG_STEPS)])  # Example: sum of martingale levels
    register_signal(pair)
    base = pair[3:6].upper()
    quote = pair[6:9].upper()
    msg = (
f"🚨HIGH CONFIDENCE TRADE!!\n\n"
...
f"💰 Maximum Gain (3 MG levels): {max_gain:.2f} units\n"
f"📊 Signal Accuracy: {score:.0f}%\n"
